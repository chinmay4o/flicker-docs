...
class Editor {
  ...
  bindDownloadButton() {
    ...
    dlButton.onclick = () => {
      if (dlButton.classList.contains("is-saving")) return; // debounce double-click
      dlButton.classList.add("is-saving");

      const textToSave = this.cm.getValue();
      const textAsBlob = new Blob([textToSave], { type: "text/plain" });
      const textAsURL = window.URL.createObjectURL(textAsBlob);
      const fileName = "Flickerdocs-" + Date.now();
      ...
      // Hold the loading state long enough to register visually with the
      // browser's download dialog rendering. After it dismisses, return to default.
      setTimeout(() => dlButton.classList.remove("is-saving"), 700);
    };
  }
  ...
  hideUploadButton(doc = document) {
    ...
    if (ulButton) ulButton.style.display = "none";
  ...
  insertText(value, positions, siteId) {
    const localCursor = this.cm.getCursor();
    const delta = this.generateDeltaFromChars(value);

    this.cm.replaceRange(value, positions.from, positions.to, "insertText");
  ...
  deleteText(value, positions, siteId) {
    ...
    this.cm.replaceRange("", positions.from, positions.to, "deleteText");
