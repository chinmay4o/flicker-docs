...
class Controller {
  ...
  attachEvents(doc = document, win = window) {
    ...
    doc
      ...
      .addEventListener("mousedown", dragModal, false);
  ...
  enableEditor(doc = document) {
    doc.getElementById("flickerdocs").classList.remove("hide");
  }
  ...
  handleSync(syncObj, doc = document, win = window) {
    ...
    syncObj.network.forEach((obj) =>
      this.addToNetwork(obj.peerId, obj.siteId, doc)
    );
  ...
  syncCompleted(peerId) {
    ...
    let connection = this.broadcast.outConns.find(
  ...
  applyOperation(operation) {
    ...
    const identifiers = char.position.map(
      (pos) => new Identifier(pos.digit, pos.siteId)
    );
